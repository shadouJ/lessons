<template>
	<div class="container mt-3 mb-5">
		<h3 class="text-center text-success mb-3">
			Several games slow: Plank length five each way
		</h3>
		<app-game-table
		...
		<div class="app--game-board mt-4">
			<div class="app--game-board-square font-weight-bold text-danger" ref="shark">Shark
				...
				<div class="badge badge-danger" v-if="result===-1" style="position: absolute; bottom: -50%; left: 0;">Shark has gobbbled you up</div>
			</div>
			<div v-for="i in plankLength" 
				:key="`shark-${i}`" 
			...
			<div class="app--game-board-square font-weight-bold text-primary" ref="middle">M
			...
			<div class="app--game-board-square font-weight-bold text-danger" ref="boat">Boat
				...
				<div class="badge badge-success" v-if="result===1" style="position: absolute; bottom: -50%;">Safe on the boat</div>
			</div>
		</div>
		<div class="app--dice">
			<div class="app--dice-direction alert alert-primary" v-if="direction!==null && step >= 1">
				{{ direction > 0 ? "Boat" : "Shark" }}
			</div>
			<div class="app--dice-walk alert alert-danger" v-if="walk!==null && step >= 2">
				{{ walk.direction > 0 ? "F" : "B"}}{{walk.steps}}
			</div>
		</div>
		<div class="app--action text-center" v-if="demoAutoOption=='0'">
			<button class="btn btn-outline-success" @click="handleRollDirection" v-if="step===0 && !result">
				Tap here to roll for direction
			</button>
			<button class="btn btn-outline-success" @click="handleRollWalk" v-if="step===1 && !result">
				Tap here to roll the walk
			</button>
			<button class="btn btn-outline-success" @click="handleWalkPlank" :disabled="isWalking" v-if="step===2 && !result">
				Tap here to walk the plank
			</button>
			<button class="btn btn-outline-success" @click="handleNextTurn" v-if="step===3 && !result">
				Tap here for next turn
			</button>
