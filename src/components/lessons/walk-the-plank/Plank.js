class Plank {
	...
	setPosition() {
		if(!this.hasDirection || !this.hasWalk) {
			...
			if(moveDirection < 0) {	// Move to shark direction
				this.position = (this.position - this.walk.steps < -1*this.length) ? -1*this.length-1 : this.position - this.walk.steps;
			} else {	// Move to boat direction
				this.position = (this.position + this.walk.steps > this.length) ? this.length+1 : this.position + this.walk.steps ;
			}
		}
	}
