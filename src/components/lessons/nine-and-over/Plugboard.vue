	...
	<div class="app--plugboard-container">
		<div class="app--plugboard">
			...
			<div class="app--plugboard-col-marker pt-2">
				<div><br><br></div>
				<div v-for="k in 10" :key="k" class="app--plugboard-marker text-primary">
					{{10-k}}
				</div>
			</div>
		</div>
...
export default {
	...
	watch: {
		...
		plugboardNumber(value, prevValue) { 
			...
			for(let i=0; i<this.plugboardNumberArr.length; i++) {
				...
				let refPlugItems = this.$refs['plugItems'+(this.plugboardNumberArr.length-i)];
				refPlugItems.forEach((item) => {
					item.classList.remove('item-clicked');
				});
				if(refPlugItems[9-this.plugboardNumberArr[i]]) {
					refPlugItems[9-this.plugboardNumberArr[i]].classList.add('item-clicked');
				}
			}
		}
	},
	methods: {
		...
		handleRecordNumber(e) {
			...
			this.$emit('setPlugboardNumber', Number(this.plugboardNumberArr.join('')));	
