			...
			<div class="col-md-6">
				<div style="display: flex; align-items: flex-start;">
					...
					<div class="pl-3">
						...
						<div v-else>
							<h6>Starting number is {{ numberSet.startingNumber }}<br>
				...
				<app-spelling-board
					:isDisable="true"
					:spellingNumber="currentSpellingNumber"
				></app-spelling-board>
				<div v-if="numberSet" class="text-center mt-3">
					<button v-if="step===1 && !isFinish" class="btn btn-outline-success" @click="handleRollDice">
						Tap here to roll dice
					</button>
					<div class="alert alert-info" v-if="step===2 && isPlugboardNumberCheckedRight === null">
						Add the dice total to the plugboard
					</div>
					<div class="alert alert-danger" v-if="step===2 && isPlugboardNumberCheckedRight === false">
						Not correct. Move the plug to count.
					</div>
					<button v-if="step===2 && !isFinish" class="btn btn-outline-success" @click="handleCheckPlugboard">
						Tap here to check
					</button>

					<div class="alert alert-info" v-if="step===3 && isCalNumberCheckedRight !== false && !isFinish ">
						Add the dice total on the calculator
					</div>
					<div class="alert alert-danger" v-if="step===3 && isCalNumberCheckedRight === false">
						Try again!
					</div>

					<div class="alert alert-danger" v-if="isFinish">Congratulations! Target reached.</div>
					<button v-if="isFinish" class="btn btn-outline-dark" @click="handleReset">
						Reset
					</button>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		handleSetNumber($event) { // In the beginning of a game, set starting number and target number
			console.log($event);
		...
		handleRollDice(){	// Roll dice
			let diffToTarget = this.currentPlugboardNumber - this.numberSet.targetNumber;
			if(diffToTarget<=6) {
		...
		handleCheckPlugboard() {	// Check number in plugboard
