	...
	<div class="app--form pl-2">
		<form>
			...
			<div class="text-center">
				<div class="badge badge-danger" v-if="startingNumber && startingNumber > maxNumber">
					Start is outside range
				</div>
				<div class="badge badge-danger" v-else-if="targetNumber && targetNumber > maxNumber">
					Target is outside range
				</div>
				<div class="badge badge-danger" v-else-if="direction > 0 && startingNumber && targetNumber && Number(targetNumber) <= Number(startingNumber)">
					Start is greater than or<br> equal to the target
				</div>
				<div class="badge badge-danger" v-else-if="direction < 0 && startingNumber && targetNumber && Number(targetNumber) >= Number(startingNumber)">
					Start is less than or<br> equal to the target
				</div>
			</div>
...
export default {
	...
	computed: {
		...
		isValidInput() {
			...
			if(Number(this.direction) > 0) {  // Addition
				 if(this.startingNumber &&
					 this.targetNumber &&
					 startingNumber >= 0 &&
