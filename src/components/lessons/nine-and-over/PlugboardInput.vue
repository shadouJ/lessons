    ...
    <div class="app--plugboard-number-container" v-for="i in 5" :key="i">
      <div class="app--plugboard-number">
        ...
        <i
          ...
          v-if="numberToCheck && digital[i-1] && numberToCheckArr[i-1]==digital[i-1]"
        ></i>
      </div>
      <!-- Input -->
    </div>
...
export default {
  ...
  watch: {
    ...
    inputNumber(value, prevValue) {
      // Change number to an array with 5 elements
      // e.g. 352 -> [0, 0, 3, 5, 2]
      if (value === null) {
        this.digital = [];
        return;
    ...
    numberToCheck(value) {
      if (value) {
        ...
        for (let i = 0; i <= 4; i++) {
          // 在前面补0以凑足指定位数
          if (Number(numberArr[numberArr.length - i - 1]) >= 0) {
            this.numberToCheckArr.unshift(
              Number(numberArr[numberArr.length - i - 1])
            );
          } else {
