				...
				<table class="table table-bordered" style="max-width: 600px;margin: 0 auto;">
					<tr>
						...
						<td class="text-center" 
							v-for="(item, index) in moveLeftArr" :key="index"
					...
					<tr>
						...
						<td class="text-center" 
							v-for="(item, index) in moveRightArr" :key="index"
							style="cursor: pointer;"
			...
			<div v-if="rule.type==='decimal'"
				...
				<div class="app--prompt">
					<h5 v-if="probMovingRight">
						Moving right <span class="badge badge-primary">{{ probMovingRight }}</span>
					</h5>
					<p v-if="!isValidProb && probMovingLeft " class="badge badge-danger mt-1">
...
export default {
	...
	computed: {
		...
		isValidProb() {	// Is input a valid probability
			if( Number(this.probMovingLeft) > 0 &&
				Number(this.probMovingLeft < 1))
				return true;
			else 
				return false;
		},
