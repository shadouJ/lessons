<template>
	<div class="mt-3 mb-5">
		<h3 class="text-success text-center">
			{{ numberOfGames }} trials for each arrangement
		</h3>
			...
			<table class="table table-bordered" style="table-layout: fixed">
				...
				<tbody>
					<tr v-for="(arrangement, index) in arrangementList" :key="index">
						...
						<td class="text-center" :class="{ 'smallest-count': smallestAverageCounts === arrangementAverageCounts[index] }">
							{{ arrangementAverageCounts[index] ? Number(arrangementAverageCounts[index].toFixed(2)) : '' }}
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="app--action">
			<button class="btn btn-outline-success" v-if="!isStart && !isEnd" @click="handleToggleTimer">
				Tap here to begin
			</button>
			<button class="btn btn-outline-success" v-if="isStart && !isEnd" @click="handleToggleTimer">
			...
			<button class="btn btn-outline-dark" v-if="isEnd" @click="handleReset">
				Reset
			</button> 
		</div>
	</div>
...
export default {
	...
	data: function() {
		return {
			...
			arrangementList: [	// List of all arrangements
				['A', 'A', 'A'],
				['B', 'B', 'B'],
				['C', 'C', 'C'],
				['A', 'A', 'B'],
	...
	computed: {
		...
		smallestAverageCounts() {
			...
			for(let i = 0; i < this.arrangementAverageCounts.length; i++) {
				if(this.arrangementAverageCounts[i] && 
	...
	methods: {
		playOnce() {
			...
			while(true) {
				...
				if(indexToRemove > -1) {	// Remove the counter
