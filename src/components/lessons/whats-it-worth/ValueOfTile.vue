<template>
	<div class="container mt-3 mb-5">
			...
			<div class="col-sm-6 text-center mb-4">
				...
				<div class="form-check">
					<input type="radio"
						...
						:disabled="shapeSelected"
					>
					<label for="type-whole-number" class="form-check-label">
						Whole number
					</label>
				</div>
				<div class="form-check">
					<input type="radio"
						...
						:disabled="shapeSelected"
					>
					<label for="type-decimals" class="form-check-label">
						Decimals
					</label>
				</div>
				<div class="form-check">
					<input type="radio"
						...
						:disabled="shapeSelected"
					>
					<label for="type-fractions" class="form-check-label">
						Fractions
					</label>
				</div>
		...
		<div class="text-center" v-if="step===1">
			...
			>
				Tap to cover the shape with tiles
			</button>
		</div>
		<div class="text-center" v-if="step>=3">
			<p>{{ shapeData.totalTiles}} cover the shape</p>
			<p>The whole shape is worth 
				<span class="text-primary font-weight-bold">
					{{ numberType==3 ? `${totalNumber.up} / ${totalNumber.down}` : totalNumber }}
				</span>
			</p>
			<p>What is each tile worth? 
				<span v-if="numberType==3">
					<input class="form-control" type="number" name="myAnswerUp" style="width: 10%; display: inline" v-model.number="myAnswerUp" > 
					/
					<input class="form-control" type="number" name="myAnswerDown" style="width: 10%; display: inline" v-model.number="myAnswerDown" >
				</span>
				<span v-else><input type="number" name="myAnswer" v-model.number="myAnswer" ></span>
			</p>
			<div class="alert alert-danger" v-if="step===4 || step===5">{{checkMessage}}</div>
			<button class="btn btn-outline-success" v-if="step===3 || step===4" @click="handleCheckAnswer">OK</button>
			<button class="btn btn-outline-success" v-if="step===5" @click="handleCreateNewShape">Tap to create a new shape</button>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		handleCheckAnswer() {
			...
			if(this.numberType === 3) {		// If number type is fraction
				...
				if(!this.myAnswerUp || !this.myAnswerDown) {
					...
					if(this.myAnswerUp/this.myAnswerDown > this.questionNumber.up/this.questionNumber.down) {
						this.checkMessage = 'Your answer is too big. Try again.';
					} else if(this.myAnswerUp/this.myAnswerDown < this.questionNumber.up/this.questionNumber.down) {
			...
			} else {		// If number type is not fraction i.e. whole number or decimals
