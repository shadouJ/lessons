<template>
	<div class="container mt-4 mb-5">
		<h3 class="text-success text-center">
			How many trials to make your favourite animal?
		</h3>
		<div class="row">
			...
			<div class="col-12 col-sm-6">
				<h4 class="text-danger">
					{{ selectedAnimalData.numberOfAnimals }} animals
				</h4>
				<h5 class="text-danger">
					Trials to make a {{ animalName }}
				</h5>
				<div class="app--stat">
					<table class="table" style="table-layout:fixed;">
						...
						<tr class="text-center">
							<td class="table-danger"><span v-if="isStart">{{ count }}</span></td>
							<td class="table-primary">count</td>
						</tr>
						<tr class="text-center">
							<td class="table-danger"><span v-if="isStart">{{ matched }}</span></td>
						...
						<tr class="text-center">
							<td class="table-danger"><span v-if="minimum > 0">{{ minimum }}</span> </td>
							<td class="table-primary">minimum</td>
						</tr>
						<tr class="text-center">
							<td class="table-danger"><span v-if="maximum > 0">{{ maximum }}</span> </td>
				...
				<div class="app--action text-center">
					...
					>
						Tap here for {{ isStart ? 'next' : 'first' }} animal
					</button>
					<button 
						class="btn btn-outline-success"
...
export default {
	...
	computed: {		
		...
		// soFar() {
		// 	if(this.matched > 0) {
		// 		return Number((this.count/this.matched).toFixed(1));
		// 	}
		// },
		minimum() {
	...
	methods: {
		...
		handleNextAnimal() {
			...
			this.count++;
			let head = getAnimalsPart(throwDiceOnce(), this.selectedAnimalData.numberOfAnimals);
			let body = getAnimalsPart(throwDiceOnce(), this.selectedAnimalData.numberOfAnimals);
			let leg = getAnimalsPart(throwDiceOnce(), this.selectedAnimalData.numberOfAnimals);
	...
	created() {
		this.animalsParts = generateAnimalsParts(this.selectedAnimalData.numberOfAnimals);
		this.getAnimalDrawn();	
	},
	destroyed() {
		clearInterval(this.timer);
