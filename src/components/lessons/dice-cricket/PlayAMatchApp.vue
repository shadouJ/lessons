<template>
	<div class="mt-4 mb-5" >
		...
		<div class="row app--score-table mt-3">
			<div class="col-lg-6">
				<h5 class="text-center" :title="matchSetting.teamAName" style="overflow: hidden;">
					{{ matchSetting.teamAName }}
				</h5>
				<app-score-table 
					:data="teamAScoreData" 
					:shouldTally="tallyTeam==='A' ? true : false"></app-score-table>
			</div>
			<div class="col-lg-6">
				<h5 class="text-center" :title="matchSetting.teamBName" style="overflow: hidden;">
					{{ matchSetting.teamBName }}
				</h5>
				<app-score-table 
					:data="teamBScoreData" 
					:shouldTally="tallyTeam==='B' ? true : false"></app-score-table>
			</div>
		</div>
		<div class="text-center mt-3" style="position: relative;">
			<div 
				...
				Coin has been tossed. Team {{ whoseTurn === 0 ? 'A' : 'B' }} to bowl first over
			</div>
			<div v-if="result"
				class="app--prompt alert alert-danger"
			>
				Match over. Team {{ result }} is the winner!
			</div>
			<button v-if="!result && demoAutoOption=='0'" class="btn btn-outline-success" @click="handlePlayMatch">
				{{ !isStart ? "Tap here for first game" : 
				 ( step===0 ? "Tap here to bowl" : "Tap here to tally") }}
			</button>
			<button v-if="!result && demoAutoOption=='1'" class="btn btn-outline-success" @click="handleToggleTimer">
				{{ !isStart ? "Tap here for first game" : 
					( timer ? "Tap here to pause" : "Tap here to resume" ) }}
			</button>
			<button v-if="result" class="btn btn-outline-dark" @click="handleReset">
				Reset
			</button>
		</div>
...
export default {
	...
	methods: {
		...
		checkResult() {		// Get the result of a game, 'A' - team A wins, 'B' - team B wins
			let [teamAWicket, teamAScore] = this.teamATally;
			let [teamBWicket, teamBScore] = this.teamBTally;
			let resultChecked = null;
			if( teamAWicket === this.maxWicket && teamBWicket > 0 && teamBWicket < this.maxWicket ) {
				//A 队到this.maxWicket ，B队没到this.maxWicket， 但B队的分数已经超过A队，这时判B胜
				if(teamBScore > teamAScore) {
					resultChecked = 'B';
				}
			} else if( teamBWicket === this.maxWicket && teamAWicket > 0 && teamAWicket < this.maxWicket ) {
				//同上B 队到this.maxWicket ，A队没到this.maxWicket， 但A队的分数已经超过B队，这时判A胜
				if(teamAScore > teamBScore) {
					resultChecked = 'A';
				}
			} else if (teamAWicket === this.maxWicket && teamBWicket === this.maxWicket) {
