...
<div>
	<div class="app--graph">
		<div 
			v-for="(item, index) in graphData" :key="index"
	...
	<div class="text-center mt-3"><span class="badge badge-light">The interval width is 10</span></div>
	<div>
		<input type="checkbox" id="check-tail" v-model="isCheckTail">
		<label for="check-tail">Check to show tail</label>&nbsp;&nbsp;
		<span v-if="isCheckTail && tail!==null">{{ tail }}% in the tail</span>
		<span v-if="isCheckTail && tail===null" class="badge badge-danger">Click on the graph to fill tail</span>
	</div>
</div>
</template>
...
export default {
	...
	computed: {
		...
		median() {
			...
			let medianNumber = inMiddle===0 ? Number(((this.gameData[middle-1] + this.gameData[middle])/2).toFixed(1)) :
	...
	methods: { 
		handleShowTail(e) {
			...
			// }
			// console.log(count);
			this.tail = Number((count/(this.gamePlayedNumber*2)*100).toFixed(2));
		}
	},
	created() {
	}
